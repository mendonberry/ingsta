...
class InstagramScraper:
    ...
    def crawl(self, max_id=None):
        ...
        for item in media['items']:
            future = self.executor.submit(self.download, item, './'
                    + self.username)
    ...
    def download(self, item, save_dir='./'):
        ...
        item['url'] = item[item['type'] + 's']['standard_resolution'
                ]['url'].split('?')[0]
...
if __name__ == '__main__':
    ...
    for future in \
        tqdm.tqdm(concurrent.futures.as_completed(scraper.future_to_item),
                  total=len(scraper.future_to_item), desc='Downloading'
                  ):
        item = scraper.future_to_item[future]

        if future.exception() is not None:
            print '%r generated an exception: %s' % (item['id'],
                    future.exception())
